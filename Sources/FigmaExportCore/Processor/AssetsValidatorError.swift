...
enum AssetsValidatorError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .countMismatchLightHighContrastColors(let light, let lightHC):
            error = "The number of assets doesn’t match. Light color palette contains theme contains \(light), and light high contrast color palette \(lightHC)."
        case .countMismatchDarkHighContrastColors(let dark, let darkHC):
            error = "The number of assets doesn’t match. Dark color palette contains theme contains \(dark), and dark high contrast color palette \(darkHC)."
